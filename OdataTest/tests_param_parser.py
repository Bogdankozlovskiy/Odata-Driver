...
TOP_SKIP_TESTS = [
    (
        "$top=" + quote("100"),
        {"slice": slice(None, 100)}
    ),
    (
        "$skip=" + quote("100"),
        {"slice": slice(100, None)}
    ),
    (
        "$top=" + quote("100") + "&" + "$skip=" + quote("50"),
        {"slice": slice(50, 100)}
    ),
]
EXPAND_TEST = [
    ("$expand=" + quote("field_a, field_b"), {"expand": ["field_a", "field_b"]}),
    ("$expand=" + quote("field_a"), {"expand": ["field_a"]}),
    ("$expand=" + quote("field_a, field_b, field_c"), {"expand": ["field_a", "field_b", "field_c"]}),
]
